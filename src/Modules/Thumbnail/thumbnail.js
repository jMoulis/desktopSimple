...
const hasSelectedTag = (selectedTags, tag) => {
  const selectedTagFound = selectedTags.find(
...
const Thumbnail = ({ user, configForThumbnail }) => {
  ...
  return (
    <div
      className={`thumbnail-container ${amISelected &&
        ...
        <ul className="ul-nav tag-list">
          {user.tags &&
            user.tags.map((tag, index) => {
              const selectedTag = hasSelectedTag(
                configForThumbnail.selectedTags,
                tag,
              );
              return (
                <li key={index}>
                  <button
                    type="button"
                    className={`tag-item
                  ${selectedTag && selectedTag === tag && 'tag-item--selected'}
                  `}
                    onClick={() =>
                      configForThumbnail.onClick({
                        spec: tag,
                        user: {
