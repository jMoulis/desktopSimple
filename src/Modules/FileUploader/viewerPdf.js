...
class ViewerPdf extends React.Component {
  ...
  componentDidMount() {
    this.renderPage(
      { value: this.state.viewer.pdfRaw },
      this.state.viewer.scaleValue,
    );
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      this.state.viewer.page !== prevState.viewer.page ||
  ...
  renderPage = (doc, scale) => {
    ViewerPdf.trimBase64(doc).then(newEncodedString => {
