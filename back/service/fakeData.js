...
function loadFakeUser() {
  while (index < numberUser) {
    const newUser = {
      ...
      school: randomData(['company', 'student']) === 'student' && `school-${index}`,
      diploma: 'Fake',
      ...
      logo: '',
      companyName: randomData(['company', 'student']) === 'company' && `company-${index}`,
      street: 'ezrzer zerz r ',
      postalCode: '45678',
      town: `town-${index}`,
      industry: randomData(['company', 'student']) === 'company' && `industry-${index}`,
      tags: randomTags(),
      teams: [],
      projects: [],
